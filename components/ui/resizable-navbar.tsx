...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  // Scroll-resize effect intentionally disabled — the navbar keeps its
  // initial look at all scroll positions.
  const visible = false;
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 text-muted-foreground dark:text-neutral-300"
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-muted dark:bg-neutral-800"
            />
          )}
...
export const MobileNavToggle = ({
  ...
  return (
    ...
    >
      {isOpen ? (
        <X className="text-foreground dark:text-white" />
      ) : (
        <Menu className="text-foreground dark:text-white" />
      )}
    </button>
  );
};
